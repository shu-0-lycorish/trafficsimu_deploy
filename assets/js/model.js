import { DELTAT_EDIE, DELTAT, VMAX } from "./constants.js"
import { init_array, draw_line, draw_rect, draw_text, draw_circle, draw_rect_edge } from "./util.js"
...
export class Link {
    ...
    draw_trafficstate() {
        ...
        var k_ref = 1
        var v_ref = 3
        draw_text("flow", this.px - 30, this.py + py0, "#B4CCF5", 12)
        draw_text("density", this.px - 30, this.py + py0 + pdy, "#B4CCF5", 12)
        draw_text("speed", this.px - 30, this.py + py0 + pdy * 2, "#B4CCF5", 12)
...
export class Spawner {
    ...
    draw() {
        draw_text("inflow = " + this.flow.toFixed(2), this.link.px, this.link.py - 20, "#B4CCF5", 12)
    }
...
export class QKplot {
    ...
    draw() {
        if (this.mfd == 0) {
            CTX.fillStyle = "#2C384C"
            CTX.fillRect(this.x0 - 40, this.y0 - this.ysize - 10, this.xsize + 50, this.ysize + 30)
            draw_line(this.x0, this.y0, this.x0 + this.xsize, this.y0, 1, "#B4CCF5")
            draw_line(this.x0, this.y0, this.x0, this.y0 - this.ysize, 1, "#B4CCF5")
            draw_text("density", this.x0 + this.xsize / 2, this.y0 + 10, "#B4CCF5", 12)
            draw_text("flow", this.x0 - 24, this.y0 - this.ysize / 2, "#B4CCF5", 12)
        } else {
            CTX.fillStyle = "#2C384C"
            CTX.fillRect(this.x0 - 70, this.y0 - this.ysize - 10, this.xsize + 90, this.ysize + 30)
            draw_line(this.x0, this.y0, this.x0 + this.xsize, this.y0, 1, "#B4CCF5")
            draw_line(this.x0, this.y0, this.x0, this.y0 - this.ysize, 1, "#B4CCF5")
            draw_text("area-density", this.x0 + this.xsize / 2, this.y0 + 10, "#B4CCF5", 12)
            draw_text("area-flow", this.x0 - 36, this.y0 - this.ysize / 2, "#B4CCF5", 12)
        }

        if (this.drawarrow == 1) {
            if (this.mfd == 0) {
                let x_to = this.link.px + this.link.plen * (this.i + this.link.deltax / 2) / this.link.xmax
                draw_line(this.x0 + this.xsize / 2, this.y0 - this.ysize - 15, x_to, this.y0 - this.ysize - 30, 1, "#B4CCF5")
                draw_line(x_to, this.y0 - this.ysize - 30, x_to, this.link.py + 30, 1, "#B4CCF5")
            }
        }
...
export class TSplot {
    ...
    draw(trajects, state, state_which) {
        var pad = 20
        draw_rect(this.px - pad * 2.5, this.py - this.pysize - pad, this.pxsize + pad * 3.5, this.pysize + pad * 2, "#2C384C")
        ...
        draw_line(this.px, this.py, this.px + this.pxsize, this.py, 1, "#B4CCF5")
        draw_line(this.px, this.py, this.px, this.py - this.pysize, 1, "#B4CCF5")
        draw_text("time", this.px + this.pxsize / 2, this.py + 12, "#B4CCF5", 12)
        draw_text("space", this.px - 24, this.py - this.pysize / 2, "#B4CCF5", 12)
    }
...
export class Cumplot {
    ...
    draw() {
        var pad = 20
        draw_rect(this.px - pad * 3.5, this.py - this.pysize - pad, this.pxsize + pad * 5.5, this.pysize + pad * 2, "#2C384C")
        ...
        for (let i = 0; i < this.xs.length; i++) {
            ...
            draw_text(this.labels[i], this.px + 45, this.py - this.pysize + 10 + 14 * i, "#B4CCF5", 12, "left")
        }

        draw_line(this.px, this.py, this.px + this.pxsize, this.py, 1, "#B4CCF5")
        draw_line(this.px, this.py, this.px, this.py - this.pysize, 1, "#B4CCF5")
        draw_text("time", this.px + this.pxsize / 2, this.py + 12, "#B4CCF5", 12)
        draw_text("cumlative", this.px - 36, this.py - this.pysize / 2 - 6, "#B4CCF5", 12)
        draw_text("count", this.px - 36, this.py - this.pysize / 2 + 6, "#B4CCF5", 12)
    }
